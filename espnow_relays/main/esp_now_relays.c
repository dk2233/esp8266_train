...
static void relays_espnow_deinit(espnow_send_param_t *send_param)
{
    ...
    free(send_param);
    vQueueDelete(example_espnow_queue);
    esp_now_deinit();
}
/* ESPNOW sending or receiving callback function is called in WiFi task.
 * Users should not do lengthy operations from this task. Instead, post
 * necessary data to a queue and handle it from a lower priority task. */
static void relays_espnow_send_cb(const uint8_t *mac_addr, esp_now_send_status_t status)
{
...
int relays_espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *state, uint16_t *seq )
{
    ...
    *state = buf->direction;
    ...
    ESP_LOGI(TAG, "Key On state %d ,key Off state %d", buf->key1_state, buf->key2_state);
...
static void relays_wifi_init(void)
{
    tcpip_adapter_init();
...
static void relays_task_esp_now(void *pvParameter)
{
    example_espnow_event_t evt;
        ...
        switch (evt.id) {
            case RELAY_ESPNOW_SEND_CB:
            {
                ...
                if (!is_broadcast) {
                    ...
                    if (send_param->count == 0) {
                        ...
                        for(int i = 0; i < ESP_NOW_ETH_ALEN; i++) 
                            ESP_LOGI(TAG, ": %xu", send_cb->mac_addr[i]);
                        ESP_LOGI(TAG, "Send done");
                        relays_espnow_deinit(send_param);
                        vTaskDelete(NULL);
                    }
                }
                ...
                if (send_param->delay > 0) {
                    ESP_LOGI(TAG, "Delay with value %d\n", send_param->delay);
                ...
                relays_espnow_data_prepare(send_param);
            ...
            case RELAY_ESPNOW_RECV_CB:
            {
                ...
                if (ret == ESPNOW_DATA_BROADCAST) {
                    ...
                    if (send_param->direction == 0) {
                        send_param->direction = 1;
                    }
                     ...
                     * ESPNOW data.
                     */
                    ESP_LOGI(TAG, "receiving state %d", recv_state);

                    if (recv_state == 1) {
                        /* The device which has the bigger magic number sends ESPNOW data, the other one
                         * receives ESPNOW data.
                         * it suits a method to distinguish from two ESP that sends data to each other which one is main and which one 
                         * is client  
                         * 
                         */
                        ESP_LOGI(TAG, "broadcast %d , unicast %d ", send_param->broadcast, send_param->unicast);
...
static esp_err_t relays_espnow_init(void)
{
    ...
    ESP_ERROR_CHECK( esp_now_register_send_cb(relays_espnow_send_cb) );
    ...
    xTaskCreate(relays_task_esp_now, "example_espnow_task", 3072, send_param, 4, NULL);
    ESP_LOGI(TAG, "ESP init\n");
